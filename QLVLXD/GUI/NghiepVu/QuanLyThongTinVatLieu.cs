...
namespace QLVLXD
{
    public partial class VatLieu : Form
    {
        ...
        void LoadDataToForm()
        {
            ...
            _BLL_VatLieu.MakeComboBox(cb_DonViTinhKM, listdvt);
        ...
        void ResetForNewInsert()
        {
            ...
            // Khuyến mãi
            tb_TenKM.Text = "<Mặc định>";
            num_SoLuongDeCoKM.Value = 0;
            num_SoLuongKM.Value = 0;
            num_TienKhuyenMai.Value = 0;
            r_SoLuongKhuyenMai.Checked = false;
            r_TienKhuyenMai.Checked = false;
            chk_CoKhuyenMai.Checked = false;
            chk_CoKhuyenMai_CheckedChanged(null, null);
            num_SoLuong.Value = 0;
        ...
        private bool CheckInput()
        {
            ...
            if (chk_CoKhuyenMai.Checked) // Có KM
            {
                if (_BLL_NhaCungCap.CheckNotInComboBox(cb_DonViTinhKM))
                {
                    MessageBox.Show("Vui lòng chọn đơn vị tính cho Khuyến mãi!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                if (cb_DonViTinhKM.Text.Trim() != cb_DonViTinh.Text.Trim()
                    && cb_DonViTinhKM.Text.Trim() != cb_DonViTinh2.Text.Trim()
                    && cb_DonViTinhKM.Text.Trim() != cb_DonViTinh3.Text.Trim())
                {
                    MessageBox.Show("Vui lòng chọn Đơn vị tính của khuyến mãi trùng với đơn vị tính gốc hoặc đơn vị tính phụ!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                if (num_SoLuongDeCoKM.Value == 0)
                {
                    MessageBox.Show("Vui lòng nhập Số lượng để có khuyến mãi lớn hơn 0!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                if ((num_SoLuongKM.Value == 0 && r_SoLuongKhuyenMai.Checked) || (r_TienKhuyenMai.Checked && num_TienKhuyenMai.Value < 1000))
                {
                    MessageBox.Show("Vui lòng nhập Số lượng khuyến mãi lớn hơn 0 hoặc Tiền khuyến mãi lớn hơn 1000!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            }
            #endregion
        ...
        {
            if (chk_CoKhuyenMai.Checked)
            {
                num_SoLuongDeCoKM.Enabled = true;
                num_SoLuongKM.Enabled = true;
                num_TienKhuyenMai.Enabled = true;
                cb_DonViTinhKM.Enabled = true;
                r_SoLuongKhuyenMai.Enabled = true;
                r_TienKhuyenMai.Enabled = true;
                tb_TenKM.Enabled = true;
            }
            else
            {
                num_SoLuongDeCoKM.Enabled = false;
                num_SoLuongKM.Enabled = false;
                num_TienKhuyenMai.Enabled = false;
                cb_DonViTinhKM.Enabled = false;
                r_SoLuongKhuyenMai.Enabled = false;
                r_TienKhuyenMai.Enabled = false;
                tb_TenKM.Enabled = false;
            }
        }
        ...
        private void grid_VatLieu_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (km != null) // Có KM
            {
                chk_CoKhuyenMai.Checked = true;
                chk_CoKhuyenMai_CheckedChanged(null, null);
                tb_TenKM.Text = _TenKMGridViewSelected;
                num_SoLuongDeCoKM.Value = (decimal)km.SoLuongToiThieu;
                cb_DonViTinhKM.Text = km.DonViTinh.Trim();
                if ((decimal)km.SoLuongKM != 0) // Km tặng sp
                {
                    r_SoLuongKhuyenMai.Checked = true;
                    r_SoLuongKhuyenMai_CheckedChanged(null, null);
                    num_SoLuongKM.Value = (decimal)km.SoLuongKM;
                }
                else
                {
                    r_TienKhuyenMai.Checked = true;
                    r_TienKhuyenMai_CheckedChanged(null, null);
                    num_TienKhuyenMai.Value = (decimal)km.TienKM;
                }
            }
            num_SoLuong.Value = (int)_VatLieuGridViewSelected.SoLuong;
